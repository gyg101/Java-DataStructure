...
| 合并排序、快速排序     | 分治算法                |                                                              |                                                              |                                                              |
| 背包问题               | 动态规划                | 列表找到状态转换方程(列表找规律)                             | dp[i] = dp[i - j* j] + 1;                                    | 将v[0][j]=0;v[i][j] = 0;if(w[i] > j){ v[i][j] = v[i -1][j]}else{ v[i][j] = max(v[i - 1][j] ,v[i] + v[i - 1][j - w[i]]); |
...
堆排序思想: 1)将无序序列构建成一个堆，根据升序或者降序需求选择大顶堆或者小顶堆；	即 先将首个非叶子结点及其左右节点构建为大顶堆
